...
# yha if user me koi input aya mtlb ki iss username, password se koi person database me h means user = not None else user= None
        if user is not None:
            if hasattr(user, 'studentprofile'):
                ...
                # this access denied error comes jb user na student, admin ho
...
def student_dashboard(request):
    ...
    subjects = StudentSubject.objects.filter(
        student=student_profile).select_related('subject')
        ...
        if collection is not None:
            ...
            for doc in collection.find({}, {
                ...
                user_obj = SimpleNamespace(
                    get_full_name=(lambda name=full_name: name),
                    email=doc.get('email', '')
                )
                mongo_students.append(SimpleNamespace(
...
def student_detail(request, student_id):
    ...
    # Initialize empty forms for adding results and attendance
