...
Making admin CRUD available on Render: run the sync script once (Render Shell or build) to create Django User + StudentProfile records from Mongo documents; afterward /admin-panel/students/ shows full View/Delete actions. Command: python scripts/sync_mongo_to_sqlite.py (or $PYTHON scripts/sync_mongo_to_sqlite.py).
Important env vars:
